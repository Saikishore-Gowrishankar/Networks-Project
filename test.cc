...
int main()
{
    ...
    while(window.isOpen())
    {
        ...
        if(update)
        {
            ...
            {
                most_recent = sf::Keyboard::W;
                shape2.move(sf::Vector2f(0.f, -2.f));
                //if(shape2.getLocalBounds().intersects())
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::A)) 
            {
                most_recent = sf::Keyboard::A;
                shape2.move(sf::Vector2f(-2.f, 0.f));
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::S)) 
            {
                most_recent = sf::Keyboard::S;

                shape2.move(sf::Vector2f(0.f, 2.f));
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::D))
            {
                most_recent = sf::Keyboard::D;
                shape2.move(sf::Vector2f(2.f, 0.f));
            }
            ...
            {
                shoot = true;
                bullet.setPosition(shape2.getPosition());
            }
            if(shoot)
            {
                bullet.move(sf::Vector2f(5.f, 0.f));
            }
            for(auto&& elem : walls)
            {
                if(shape2.getGlobalBounds().intersects(elem))
                    shape2.setPosition(prevPosition);
                if(bullet.getGlobalBounds().intersects(elem))
                    shoot = false;
            }
